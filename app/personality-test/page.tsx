...
export default function PersonalityTest() {
  ...
  useEffect(() => {
    // Check if this is a paid test
    const urlParams = new URLSearchParams(window.location.search)
    setIsPaidTest(urlParams.get("paid") === "true")
  }, [])

  const handleAnswer = (score: number) => {
    setAnswers((prev) => [...prev, { questionId: personalityQuestions[currentQuestion].id, score }])
  ...
  const calculatePersonalityType = () => {
    ...
    // Store the personality type and test date in localStorage for demo purposes
          ...
          <CardContent className="space-y-4">
            ...
            {isPaidTest && (
              <p className="text-sm text-muted-foreground text-center">
                This is a paid retake of the personality test.
              </p>
            )}
          </CardContent>
        </Card>
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl mt-8">
        ...
        <QuestionCard
          ...
          currentAnswer={answers.find((a) => a.questionId === personalityQuestions[currentQuestion].id)?.score || null}
